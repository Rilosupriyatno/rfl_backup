...
function updateTotalChecked() {
  var checkboxes = document.querySelectorAll('input[type="checkbox"][name^="check-list-product"]:checked');
  var total = 0;
  var orderDetails = [];

  checkboxes.forEach(function(checkbox) {
      var parentDiv = checkbox.closest('.flex.flex-col.gap-5.border-t-2.py-4.px-6');

      var orderDetailIdInput = parentDiv.querySelector('input[name="order_detail_id"]');
      if (orderDetailIdInput) {
      var orderDetailId = orderDetailIdInput.value;
      var qtyInput = parentDiv.querySelector('.add-remove-container input[name="qty"]');
      ...
      var priceElement = parentDiv.querySelector('.flex.items-center.justify-between span.font-bold');
      var qtyInput = parentDiv.querySelector('.add-remove-container input[name="qty"]');

      if (priceElement && qtyInput) {
      var price = parseFloat(priceElement.innerText.replace('Rp. ', '').replace(/\./g, '').replace(',', '.'));
      var qty = parseInt(qtyInput.value);
  ...
  $('#total-barang').text('Total Harga (' + checkboxes.length + ' Barang)');
  ...
  function updateHiddenFields(orderDetails) {
    ...
    orderDetails.forEach(function(orderDetail) {
        var orderDetailId = orderDetail.orderDetailId;
    ...
    document.getElementById('order_detail_id_inputter').value = orderDetailIds.join(',');
    document.getElementById('qty_inputter').value = qtys.join(',');
    }

    function displayFormData() {
    var orderDetailId = $('#order_detail_id_inputter').val();
    var qty = $('#qty_inputter').val();

    // Set the values of the input fields
    $('input[name="order_detail_id_inputter[]"]').val(orderDetailId);
    $('input[name="qty_inputter[]"]').val(qty);
    }
...
var _require = __webpack_require__(/*! ../../helper/add-remove-qty */ "./resources/js/helper/add-remove-qty.js"),
  addRemoveQty = _require.addRemoveQty;
window.addEventListener("load", function () {
  var addRemoveQtyEl = document.querySelectorAll(".add-remove-container");
  addRemoveQty(addRemoveQtyEl);
});
})();
